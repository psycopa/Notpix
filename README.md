# langkah pertama
```
git clone https://github.com/psycopa/Notpix
```
```
cd Notpix
```
```
pip install -r requirements.txt
```
# lengkapi api dan hash kalian di sesi.py
dapetin data itu di my.telegram.org
```
nano sesi.py
```
ganti xxxxxx dengan data kalian
( PERHATIKAN TANDA JNGN SMPAI SALAH )
# lalu save dengan ctrl+x+y lalu enter

# run bot 
```
python sesi.py
```
• masukan nama sesi ( bebas )

• no hp awalan +62...

• OTP

• password ( jika ada )

# jika mau pakai template lain ? gunakan perintah ini !
• buka bot di web > mode developer > pilih fetch/XHR

• lalu pilih file my > preview > simpan data

• download gambarnya salin bagian url > paste on web > download

• lalu ganti nama gambar tadi ubah ke 111.png > dan masukan ke dalam file script

• next run bot create template dengan perintah ini

```
python create.py
```

• masukan data yang anda dapatkan di preview tadi 

• lalu salin data yang di dapatkan setelah run bot berupa ( , , ,)

# lalu masukan data ke dalam bot dengan cara
```
nano sesi.py
```
• lalu cari areas dengan cara CTRL+f ( tulis areas ) enter tempel disitu

• dan masukan id template di bagian ( def pain_pixel )

• CONTOH https://notpx.app/api/v1/image/template/subscribe/1467975528

• ubah nomer id setelah subscribe/ dan cari disitu total ada 5 yang perlu di ubah


# THANKS FOR ATTENTITON

buy me a coffe

EVM : 0x30611dA354E5732B5d849951bC69d5d41296ddf1
